#include "FuelManagement__type.h"
...
#ifdef __cplusplus
  #ifdef pSimBoolMsgVTable_defined
    extern struct SimTypeVTable *pSimBoolMsgVTable;
  #else 
    struct SimTypeVTable *pSimBoolMsgVTable = NULL;
  #endif
#else
  struct SimTypeVTable *pSimBoolMsgVTable;
#endif

static SimFieldUtils BoolMsg_fields[] = {
    {"Emit", offsetof(BoolMsg,Emit), &_Type_kcg_bool_Utils},
    {"Value", offsetof(BoolMsg,Value), &_Type_kcg_bool_Utils},
};

int BoolMsg_to_string(const void *pValue, PFN_STR_APPEND pfnStrAppend, void *pStrObj)
{
    if (pSimBoolMsgVTable != NULL
        && pSimBoolMsgVTable->m_pfnGetConvInfo(SptString, SptNone) == 1) {
       return pfnStrAppend(*(char**)pSimBoolMsgVTable->m_pfnToType(SptString, pValue), pStrObj);
    }
    return pConverter->m_pfnStructureToString(pValue, BoolMsg_fields, 2, pfnStrAppend, pStrObj);
}

int check_BoolMsg_string(const char *str, char **endptr)
{
    static BoolMsg rTemp;
    return string_to_BoolMsg(str, &rTemp, endptr);
}

int string_to_BoolMsg(const char *str, void *pValue, char **endptr)
{
    int nRet = 0;
    skip_whitespace(str);
    if (pSimBoolMsgVTable != NULL) {
        nRet = string_to_VTable(str, pSimBoolMsgVTable, pValue, endptr);
    }
    if (nRet == 0) {
        nRet = pConverter->m_pfnStringToStructure(str, pValue, BoolMsg_fields, 2, endptr);
    }
    return nRet;
}

int is_BoolMsg_double_conversion_allowed()
{
    if (pSimBoolMsgVTable != NULL) {
        return is_VTable_double_conversion_allowed(pSimBoolMsgVTable);
    }
    return 0;
}

int BoolMsg_to_double(const void *pValue, double *nValue)
{
    if (pSimBoolMsgVTable != NULL) {
        return VTable_to_double(pValue, pSimBoolMsgVTable, nValue);
    }
    return 0;
}

int is_BoolMsg_long_conversion_allowed()
{
    if (pSimBoolMsgVTable != NULL) {
        return is_VTable_long_conversion_allowed(pSimBoolMsgVTable);
    }
    return 0;
}

int BoolMsg_to_long(const void *pValue, long *nValue)
{
    if (pSimBoolMsgVTable != NULL) {
        return VTable_to_long(pValue, pSimBoolMsgVTable, nValue);
    }
    return 0;
}

void compare_BoolMsg(int *pResult, const void *pValue1, const void *pValue2, SimTolerance *pTol, const char *pszPath, PFN_STR_LIST_APPEND pfnStrListAppend, void *pListErrPaths)
{
    int unitResult = 0;
    /* Customized comparison */
    if (pSimBoolMsgVTable != NULL
        && pSimBoolMsgVTable->m_version >= Scv612
        && pSimBoolMsgVTable->m_pfnCompare != NULL) {
        if (pSimBoolMsgVTable->m_version >= Scv65) {
            /* R15 and higher: VTable Compare function shall UPDATE *pResult global flag (*pResult |= SIM_CMP_RES_LT/...): */
            unitResult = pSimBoolMsgVTable->m_pfnCompare(pResult, pValue1, pValue2);
        } else {
            /* Before R15: VTable Compare function shall SET *pResult global flag (*pResult = -1/1/0): */
            pSimBoolMsgVTable->m_pfnCompare(&unitResult, pValue1, pValue2);
            updateCompareResult(unitResult, pResult);
        }
    } else {
        /* Predefined comparison */
        pConverter->m_pfnStructureComparison(pResult, pValue1, pValue2, BoolMsg_fields, 2, pTol, pszPath, pfnStrListAppend, pListErrPaths);
    }
    if (unitResult != 0 && pfnStrListAppend != NULL && pszPath != NULL && *pszPath != 0 && pListErrPaths != NULL)
        pfnStrListAppend(pszPath, pListErrPaths);
}

int get_BoolMsg_signature(PFN_STR_APPEND pfnStrAppend, void *pStrObj)
{
    return pConverter->m_pfnGetStructureSignature(BoolMsg_fields, 2, pfnStrAppend, pStrObj);
}

int init_BoolMsg(void *pValue)
{
    init_kcg_bool(&(((BoolMsg*)pValue)->Emit));
    init_kcg_bool(&(((BoolMsg*)pValue)->Value));
    return 1;
}

int release_BoolMsg(void *pValue)
{
    UNUSED(pValue);
    return 1;
}

int copy_BoolMsg(void *pToValue, const void *pFromValue)
{
    kcg_copy_BoolMsg((BoolMsg*)pToValue, (BoolMsg*)pFromValue);
    return 1;
}

SimTypeUtils _Type_BoolMsg_Utils = {
    BoolMsg_to_string,
    check_BoolMsg_string,
    string_to_BoolMsg,
    is_BoolMsg_double_conversion_allowed,
    BoolMsg_to_double,
    is_BoolMsg_long_conversion_allowed,
    BoolMsg_to_long,
    compare_BoolMsg,
    get_BoolMsg_signature,
    init_BoolMsg,
    release_BoolMsg,
    copy_BoolMsg,
    sizeof(BoolMsg)
...
#ifdef __cplusplus
  #ifdef pSimintMsgVTable_defined
    extern struct SimTypeVTable *pSimintMsgVTable;
  #else 
    struct SimTypeVTable *pSimintMsgVTable = NULL;
  #endif
#else
  struct SimTypeVTable *pSimintMsgVTable;
#endif

static SimFieldUtils intMsg_fields[] = {
    {"Emit", offsetof(intMsg,Emit), &_Type_kcg_bool_Utils},
    {"Value", offsetof(intMsg,Value), &_Type_kcg_uint32_Utils},
};

int intMsg_to_string(const void *pValue, PFN_STR_APPEND pfnStrAppend, void *pStrObj)
{
    if (pSimintMsgVTable != NULL
        && pSimintMsgVTable->m_pfnGetConvInfo(SptString, SptNone) == 1) {
       return pfnStrAppend(*(char**)pSimintMsgVTable->m_pfnToType(SptString, pValue), pStrObj);
    }
    return pConverter->m_pfnStructureToString(pValue, intMsg_fields, 2, pfnStrAppend, pStrObj);
}

int check_intMsg_string(const char *str, char **endptr)
{
    static intMsg rTemp;
    return string_to_intMsg(str, &rTemp, endptr);
}

int string_to_intMsg(const char *str, void *pValue, char **endptr)
{
    int nRet = 0;
    skip_whitespace(str);
    if (pSimintMsgVTable != NULL) {
        nRet = string_to_VTable(str, pSimintMsgVTable, pValue, endptr);
    }
    if (nRet == 0) {
        nRet = pConverter->m_pfnStringToStructure(str, pValue, intMsg_fields, 2, endptr);
    }
    return nRet;
}

int is_intMsg_double_conversion_allowed()
{
    if (pSimintMsgVTable != NULL) {
        return is_VTable_double_conversion_allowed(pSimintMsgVTable);
    }
    return 0;
}

int intMsg_to_double(const void *pValue, double *nValue)
{
    if (pSimintMsgVTable != NULL) {
        return VTable_to_double(pValue, pSimintMsgVTable, nValue);
    }
    return 0;
}

int is_intMsg_long_conversion_allowed()
{
    if (pSimintMsgVTable != NULL) {
        return is_VTable_long_conversion_allowed(pSimintMsgVTable);
    }
    return 0;
}

int intMsg_to_long(const void *pValue, long *nValue)
{
    if (pSimintMsgVTable != NULL) {
        return VTable_to_long(pValue, pSimintMsgVTable, nValue);
    }
    return 0;
}

void compare_intMsg(int *pResult, const void *pValue1, const void *pValue2, SimTolerance *pTol, const char *pszPath, PFN_STR_LIST_APPEND pfnStrListAppend, void *pListErrPaths)
{
    int unitResult = 0;
    /* Customized comparison */
    if (pSimintMsgVTable != NULL
        && pSimintMsgVTable->m_version >= Scv612
        && pSimintMsgVTable->m_pfnCompare != NULL) {
        if (pSimintMsgVTable->m_version >= Scv65) {
            /* R15 and higher: VTable Compare function shall UPDATE *pResult global flag (*pResult |= SIM_CMP_RES_LT/...): */
            unitResult = pSimintMsgVTable->m_pfnCompare(pResult, pValue1, pValue2);
        } else {
            /* Before R15: VTable Compare function shall SET *pResult global flag (*pResult = -1/1/0): */
            pSimintMsgVTable->m_pfnCompare(&unitResult, pValue1, pValue2);
            updateCompareResult(unitResult, pResult);
        }
    } else {
        /* Predefined comparison */
        pConverter->m_pfnStructureComparison(pResult, pValue1, pValue2, intMsg_fields, 2, pTol, pszPath, pfnStrListAppend, pListErrPaths);
    }
    if (unitResult != 0 && pfnStrListAppend != NULL && pszPath != NULL && *pszPath != 0 && pListErrPaths != NULL)
        pfnStrListAppend(pszPath, pListErrPaths);
}

int get_intMsg_signature(PFN_STR_APPEND pfnStrAppend, void *pStrObj)
{
    return pConverter->m_pfnGetStructureSignature(intMsg_fields, 2, pfnStrAppend, pStrObj);
}

int init_intMsg(void *pValue)
{
    init_kcg_bool(&(((intMsg*)pValue)->Emit));
    init_kcg_uint32(&(((intMsg*)pValue)->Value));
    return 1;
}

int release_intMsg(void *pValue)
{
    UNUSED(pValue);
    return 1;
}

int copy_intMsg(void *pToValue, const void *pFromValue)
{
    kcg_copy_intMsg((intMsg*)pToValue, (intMsg*)pFromValue);
    return 1;
}

SimTypeUtils _Type_intMsg_Utils = {
    intMsg_to_string,
    check_intMsg_string,
    string_to_intMsg,
    is_intMsg_double_conversion_allowed,
    intMsg_to_double,
    is_intMsg_long_conversion_allowed,
    intMsg_to_long,
    compare_intMsg,
    get_intMsg_signature,
    init_intMsg,
    release_intMsg,
    copy_intMsg,
    sizeof(intMsg)
...
#ifdef __cplusplus
  #ifdef pSimintMsg2VTable_defined
    extern struct SimTypeVTable *pSimintMsg2VTable;
  #else 
    struct SimTypeVTable *pSimintMsg2VTable = NULL;
  #endif
#else
  struct SimTypeVTable *pSimintMsg2VTable;
#endif

static SimFieldUtils intMsg2_fields[] = {
    {"Emit", offsetof(intMsg2,Emit), &_Type_kcg_bool_Utils},
    {"Value", offsetof(intMsg2,Value), &_Type_kcg_uint8_Utils},
};

int intMsg2_to_string(const void *pValue, PFN_STR_APPEND pfnStrAppend, void *pStrObj)
{
    if (pSimintMsg2VTable != NULL
        && pSimintMsg2VTable->m_pfnGetConvInfo(SptString, SptNone) == 1) {
       return pfnStrAppend(*(char**)pSimintMsg2VTable->m_pfnToType(SptString, pValue), pStrObj);
    }
    return pConverter->m_pfnStructureToString(pValue, intMsg2_fields, 2, pfnStrAppend, pStrObj);
}

int check_intMsg2_string(const char *str, char **endptr)
{
    static intMsg2 rTemp;
    return string_to_intMsg2(str, &rTemp, endptr);
}

int string_to_intMsg2(const char *str, void *pValue, char **endptr)
{
    int nRet = 0;
    skip_whitespace(str);
    if (pSimintMsg2VTable != NULL) {
        nRet = string_to_VTable(str, pSimintMsg2VTable, pValue, endptr);
    }
    if (nRet == 0) {
        nRet = pConverter->m_pfnStringToStructure(str, pValue, intMsg2_fields, 2, endptr);
    }
    return nRet;
}

int is_intMsg2_double_conversion_allowed()
{
    if (pSimintMsg2VTable != NULL) {
        return is_VTable_double_conversion_allowed(pSimintMsg2VTable);
    }
    return 0;
}

int intMsg2_to_double(const void *pValue, double *nValue)
{
    if (pSimintMsg2VTable != NULL) {
        return VTable_to_double(pValue, pSimintMsg2VTable, nValue);
    }
    return 0;
}

int is_intMsg2_long_conversion_allowed()
{
    if (pSimintMsg2VTable != NULL) {
        return is_VTable_long_conversion_allowed(pSimintMsg2VTable);
    }
    return 0;
}

int intMsg2_to_long(const void *pValue, long *nValue)
{
    if (pSimintMsg2VTable != NULL) {
        return VTable_to_long(pValue, pSimintMsg2VTable, nValue);
    }
    return 0;
}

void compare_intMsg2(int *pResult, const void *pValue1, const void *pValue2, SimTolerance *pTol, const char *pszPath, PFN_STR_LIST_APPEND pfnStrListAppend, void *pListErrPaths)
{
    int unitResult = 0;
    /* Customized comparison */
    if (pSimintMsg2VTable != NULL
        && pSimintMsg2VTable->m_version >= Scv612
        && pSimintMsg2VTable->m_pfnCompare != NULL) {
        if (pSimintMsg2VTable->m_version >= Scv65) {
            /* R15 and higher: VTable Compare function shall UPDATE *pResult global flag (*pResult |= SIM_CMP_RES_LT/...): */
            unitResult = pSimintMsg2VTable->m_pfnCompare(pResult, pValue1, pValue2);
        } else {
            /* Before R15: VTable Compare function shall SET *pResult global flag (*pResult = -1/1/0): */
            pSimintMsg2VTable->m_pfnCompare(&unitResult, pValue1, pValue2);
            updateCompareResult(unitResult, pResult);
        }
    } else {
        /* Predefined comparison */
        pConverter->m_pfnStructureComparison(pResult, pValue1, pValue2, intMsg2_fields, 2, pTol, pszPath, pfnStrListAppend, pListErrPaths);
    }
    if (unitResult != 0 && pfnStrListAppend != NULL && pszPath != NULL && *pszPath != 0 && pListErrPaths != NULL)
        pfnStrListAppend(pszPath, pListErrPaths);
}

int get_intMsg2_signature(PFN_STR_APPEND pfnStrAppend, void *pStrObj)
{
    return pConverter->m_pfnGetStructureSignature(intMsg2_fields, 2, pfnStrAppend, pStrObj);
}

int init_intMsg2(void *pValue)
{
    init_kcg_bool(&(((intMsg2*)pValue)->Emit));
    init_kcg_uint8(&(((intMsg2*)pValue)->Value));
    return 1;
}

int release_intMsg2(void *pValue)
{
    UNUSED(pValue);
    return 1;
}

int copy_intMsg2(void *pToValue, const void *pFromValue)
{
    kcg_copy_intMsg2((intMsg2*)pToValue, (intMsg2*)pFromValue);
    return 1;
}

SimTypeUtils _Type_intMsg2_Utils = {
    intMsg2_to_string,
    check_intMsg2_string,
    string_to_intMsg2,
    is_intMsg2_double_conversion_allowed,
    intMsg2_to_double,
    is_intMsg2_long_conversion_allowed,
    intMsg2_to_long,
    compare_intMsg2,
    get_intMsg2_signature,
    init_intMsg2,
    release_intMsg2,
    copy_intMsg2,
    sizeof(intMsg2)
...
static SimEnumValUtils SSM_ST_SM1_values[] = {
    { "TankClosed", SSM_st_TankClosed_SM1},
    { "TankClosed", SSM_st_TankClosed_SM1},
    { "TankOpen", SSM_st_TankOpen_SM1},
    { "TankOpen", SSM_st_TankOpen_SM1},
};
...
{
    *(SSM_ST_SM1*)pValue = SSM_st_TankClosed_SM1;
    return 1;
}
...
static SimEnumValUtils SSM_TR_SM1_values[] = {
    ...
    { "SSM_TR_no_trans_SM1", SSM_TR_no_trans_SM1},
    { "SSM_TR_TankClosed_TankOpen_1_TankClosed_SM1", SSM_TR_TankClosed_TankOpen_1_TankClosed_SM1},
    { "SSM_TR_TankClosed_TankOpen_1_TankClosed_SM1", SSM_TR_TankClosed_TankOpen_1_TankClosed_SM1},
    { "SSM_TR_TankOpen_TankClosed_1_TankOpen_SM1", SSM_TR_TankOpen_TankClosed_1_TankOpen_SM1},
    { "SSM_TR_TankOpen_TankClosed_1_TankOpen_SM1", SSM_TR_TankOpen_TankClosed_1_TankOpen_SM1},
};
